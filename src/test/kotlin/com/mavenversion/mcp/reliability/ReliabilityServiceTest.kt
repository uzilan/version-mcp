...
class ReliabilityServiceTest {
    ...
    inner class RetryLogicTests {
        ...
        fun shouldSucceedOnFirstAttempt() = runTest {
            val service = ReliabilityService(maxRetries = 3, baseDelayMs = 100, rateLimitDelayMs = 0)
            var attempts = 0

            val result = service.executeWithRetry("test operation") {
                attempts++
                "success"
            }
        ...
        fun shouldRetryOnRetryableExceptions() = runTest {
        ...
        fun shouldNotRetryOnNonRetryableExceptions() = runTest {
            val service = ReliabilityService(maxRetries = 3, baseDelayMs = 50, rateLimitDelayMs = 0)
            var attempts = 0
        ...
        fun shouldApplyExponentialBackoff() = runTest {
            val service = ReliabilityService(maxRetries = 3, baseDelayMs = 100, rateLimitDelayMs = 0)
            var attempts = 0
    ...
    inner class RateLimitingTests {
        ...
        fun shouldApplyRateLimitingBetweenRequests() = runTest {
        ...
        fun shouldSkipRateLimitingWhenDisabled() = runTest {
    ...
    inner class WebsiteStructureErrorHandlingTests {
        ...
        fun shouldHandleStructureChangeErrorsGracefully() {
            ...
            val result = service.handleStructureChangeError(
                "test operation",
                ".test-selector",
                originalException
            )
